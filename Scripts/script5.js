...
function inLista(n, l){
    if(l.indexOf(Number(n)) != -1){ //-1 indica que o valor nao foi encontrado na lista valores
        return true
    }else{
        return false
    }
...
function finalizar(){
    if (valores.length == 0){
        ...
        for(let pos in valores){
            soma += valores[pos] // soma os valores nas posiçoes
            if(valores[pos] > maior){//se o valore da posição é maior que o let maior ele assume o valor da posição
                maior = valores[pos]
            }
